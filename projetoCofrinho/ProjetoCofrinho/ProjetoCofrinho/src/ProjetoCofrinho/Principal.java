...
public class Principal {
	public static void main(String[] args) {
		...
		while(true) {
			...
			/*verifica se o numero que foi digitado pelo usuario e um numero entre 0 e 5*/
			int opcao;
			do {
                while (!teclado.hasNextInt()) { //verifica se é inteiro o valor digitado
                    System.out.println("Erro! Essa opcão não existe: ");
                    System.out.println("Tente novamente: ");
                    teclado.next(); //limpa o buffer de entrada
                }
                opcao = teclado.nextInt();
                if (opcao <= 0 || opcao > 5) { //verifica se esta no intervalo de 1 a 5 o valor
                    System.out.println("Erro! Essa opcao nao existe: ");
                    System.out.println("Tente novamente: ");
                }
            } while (opcao <= 0 || opcao > 5); //repete até que o valor esteja entre 1 e 5
